...
const Todo = () => {
    const [input, setInput] = useState("");    // whatever user type stores in input
    // const [items, setItems] = useState([]);
    const [items, setItems] = useState(getLocalItems());  // array of all todo
    ...
    return (
        <>
            <div className="mainDiv offset-2 col-10 container-fluid d-flex align-items-center justify-content-center bg-secondary">
                <div className="innerDiv h-75 pt-5">
                    ...
                    <div className="inputBox d-flex mb-4">
                        ...
                        {toggle ? <AddOutlinedIcon className="addIcon mt-1" onClick={addItem} /> : <EditIcon className="text-success editIcon" id="editIconInputBox" onClick={addItem} />}
                    </div>
                    {
                        items.map((curElem) => {
                            return (
